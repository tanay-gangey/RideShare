...
def spawnWorker():
    ...
    for image in containerList:
        if(image.attrs['Config']['Image'] not in ['zookeeper', 'python', 'rabbitmq', 'common_orchestrator']):
    ...
    if numContainers > workers:
        ...
        while extra:
            ...
            dockEnv.containers.run("common_slave", ["python3", "slave.py"], links={
                                   "rmq": "rmq"}, network="common_default", detach=True)
            numContainers += 1
            extra -= 1
...
def getWorkers():
    if request.method == "GET":
        ...
        pidlist = list()
        for image in containerList:
            if image['Config']['Image'] not in ['zookeeper', 'python', 'rabbitmq', 'common_orchestrator']:
